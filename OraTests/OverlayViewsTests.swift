...
final class OverlayViewsTests: XCTestCase {
    ...
    func test_chatBubbleView_bodyBuilds_withUnifiedGlassBackgrounds() {
        // Test all roles with reduceTransparency: false (unified glass path)
        // This exercises the bubbleBackgroundColor code path
    ...
    func test_chatBubbleView_bodyBuilds_withReduceTransparencyBackgrounds() {
        // Test all roles with reduceTransparency: true (solid background path)
        let userBubble = ChatBubbleView(
            text: "User message",
            role: .user,
            state: nil,
