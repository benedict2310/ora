...
- `Ora/Orchestration/AgentLoop.swift` - Added `AgentActivity` enum, extended `AgentLoopDelegate` with `didUpdateActivity`, emits activity events during processing, added `currentActivity` dedup property
...
#### P1 - Major (Should fix)
- [x] `Ora/Orchestration/AgentLoop.swift` - Fixed. `.composing` is now emitted on first token using `currentActivity` dedup logic.
...
- [x] PR merged: https://github.com/benedict2310/ora/pull/73
- [x] Merged to main: 93f0654
- [x] Date: 2026-01-17
