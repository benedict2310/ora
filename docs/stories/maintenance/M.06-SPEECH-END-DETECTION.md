...
3. Add "finalize fallbacks":
   - No-change timeout: if ASR text unchanged for 1.0s → finalize
...
| `test_noChangeTimeout_finalizesAfterStableText` | Finalize after 1.0s of unchanged text |
...
### Finalize Fallbacks
- [x] AC-7: No-change timeout finalizes after 1.0s of stable text - ✅ `SilenceDetector.noChangeTimeout = 1.0s`, implemented in `startNoChangeTimer()`
